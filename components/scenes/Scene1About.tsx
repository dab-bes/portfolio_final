  ...
  return (
    <div className="relative flex min-h-0 w-full flex-1 flex-col gap-8 md:flex-row md:items-stretch md:gap-10">
      <div className="flex min-w-0 flex-1 flex-col md:gap-0">
        <div className="flex max-md:mt-7 max-md:items-center max-md:justify-between max-md:gap-3 md:contents">
          <h1 className="animate-scene-in-title relative z-20 min-w-0 max-md:flex-1 font-brand text-3xl font-thin normal-case tracking-wide md:text-4xl">
            {heading}
          </h1>
        ...
        <div className="relative max-md:mt-4 md:mt-6">
          <div className="flex max-md:flex-col max-md:items-end max-md:gap-2 md:contents">
            <div className="animate-scene-about-in-bio relative z-10 w-full rounded-lg border border-white/15 bg-black/25 px-4 py-3 shadow-sm backdrop-blur-sm max-md:flex max-md:h-[min(65vh,26rem)] max-md:min-h-0 max-md:flex-col max-md:overflow-hidden">
              <div className="hide-scrollbar min-h-0 max-md:min-h-0 max-md:flex-1 max-md:overflow-y-auto max-md:overscroll-y-contain">
                <h2 className="mb-5 border-b border-white/15 pb-3 text-right font-brand text-lg font-light italic tracking-[0.02em] text-white md:text-xl">
                  {BIO_STORY_TITLE}
                </h2>
                {Array.isArray(body) ? (
            ...
            <a
              ...
              className="animate-scene-about-in-resume z-20 inline-flex shrink-0 border-b border-white/35 px-0 pb-px font-nav text-[0.625rem] font-light lowercase tracking-wide text-white transition-[opacity,border-color] hover:border-white/55 hover:opacity-80 active:opacity-70 md:hidden"
            >
              resume
            </a>
          </div>
      ...
      <a
        href={RESUME_HREF}
        download={RESUME_FILE}
        className="animate-scene-about-in-resume absolute top-0 right-0 z-10 hidden border-b border-white/35 px-0 pb-px font-nav text-xs font-light lowercase tracking-wide text-white transition-[opacity,border-color] hover:border-white/55 hover:opacity-80 active:opacity-70 md:inline-flex"
      >
        download resume
      </a>
    </div>
